            ...
            try:
                ...
                try:
                    if self.evaluator is not None:
                        if hasattr(self.evaluator, "update_distance_scales"):
                            self.evaluator.update_distance_scales(
                                np.asarray(actual_objectives, dtype=np.float64).reshape(1, -1),
                                reset=False,
                            )
                        objective_outputs = self.evaluator.compute_objective_outputs(
                            actual_objectives,
                            force_target_normalization=self.uses_direct_objectives,
                        )
                        if self.uses_direct_objectives:
        ...
        try:
            ...
            if self.evaluator_type == EvaluatorType.SELF_DRIVING_LAB and self.sdl_client is not None:
                ...
                if self.evaluator is not None:
                    if hasattr(self.evaluator, "update_distance_scales"):
                        self.evaluator.update_distance_scales(
                            np.asarray(measured_array, dtype=np.float64).reshape(1, -1),
                            reset=False,
                        )
                    objective_outputs = self.evaluator.compute_objective_outputs(
                        measured_array,
            ...
            if hasattr(self.evaluator, "update_distance_scales"):
                self.evaluator.update_distance_scales(
                    np.asarray(prediction_for_distance, dtype=np.float64).reshape(1, -1),
                    reset=False,
                )
            objective_outputs = self.evaluator.compute_objective_outputs(
