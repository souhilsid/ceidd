...
class BOPlatform:
    ...
    def _resolve_distance_normalization_config(
        ...
        # Legacy keys kept for backward compatibility in old configs; ignored now.
        resolved.pop("method", None)
        resolved.pop("q_low", None)
        resolved.pop("q_high", None)
        resolved.pop("min_scale", None)
        resolved.pop("normalize_target_components", None)
        resolved.pop("max_scale_samples", None)
        return resolved
...
# Optimization settings - UPDATED WITH FULLYBAYESIAN
        ...
        if target_count > 0:
            ...
            with dn_col1:
                distance_normalization_config["enabled"] = bool(st.checkbox(
                    ...
                    help="Apply distance post-processing controls before weighted Euclidean aggregation.",
                ))
            ...
            with dn_col3:
                if direct_count > 0:
                    st.caption(
                        "`Scale Method` was removed. Direct objectives are no longer auto-rescaled here."
                    )
                else:
                    st.caption(
    ...
    def render_optimization(self, show_header: bool = True):
        ...
        **Custom Models**: {'Yes' if has_custom_models else 'No'}
        ...
        if checkpoint_data:
            ...
            if st.button("Export loaded checkpoint history CSV", use_container_width=True, key="export_loaded_checkpoint_csv_btn"):
                try:
                    export_stamp = int(time.time())
                    loaded_name = "loaded_checkpoint"
                    ...
                    st.success(f"Checkpoint history CSVs exported: {csv_paths.get('candidates_csv')}")
                    for label, csv_path in csv_paths.items():
                        try:
                            ...
                            st.download_button(
                                ...
                                key=f"download_loaded_{label}_{Path(csv_path).name}",
                            )
                        except Exception as dl_err:
                            st.warning(f"Could not prepare download for {label}: {dl_err}")
                except Exception as exp_err:
                    st.error(f"Failed to export loaded checkpoint CSVs: {exp_err}")
        ...
        if save_clicked or pause_clicked or stop_clicked:
            if 'optimizer' in st.session_state:
                ...
                csv_paths = (st.session_state.get('checkpoint_info') or {}).get('csv_paths', {})
                if isinstance(csv_paths, dict) and csv_paths:
                    st.caption("History CSVs were saved with this checkpoint.")
                    for label, csv_path in csv_paths.items():
                        try:
                            csv_bytes = open(csv_path, "rb").read()
                            st.download_button(
                                ...
                                file_name=Path(csv_path).name,
                                use_container_width=True,
                                key=f"download_saved_{label}_{Path(csv_path).name}",
                            )
                        except Exception as dl_err:
                            st.warning(f"Could not prepare {label} download: {dl_err}")
                if stop_clicked:
    ...
    def _run_optimization(self, config, X, Y, Y_sem=None, resume_state=None, single_batch=True, sdl_connector=None, reset_progress=True):
        ...
        try:
            ...
            self._update_virtual_model_performance_state(
                st.session_state.optimization_progress,
            ...
            def update_progress(progress_info):
                ...
                if status == 'starting_batch':
                    ...
                    self._update_virtual_model_performance_state(
                        current_progress,
                        optimizer=self.optimizer,
                        best_models=progress_info.get('best_models_used', {}),
                        record_history=True,
                    )
                try:
    ...
    def _save_checkpoint(self, config: OptimizationConfig, X: np.ndarray, Y: np.ndarray, Y_sem: Optional[np.ndarray] = None, optimizer: Optional[BayesianOptimizer] = None, progress: Optional[Dict[str, Any]] = None, filename: Optional[str] = None) -> str:
        ...
        try:
            csv_paths = export_checkpoint_csvs(payload, output_prefix=os.path.splitext(path)[0])
        except Exception as e:
            print(f"Warning: could not export checkpoint CSV history: {e}")

        checkpoint_info: Dict[str, Any] = {'path': path, 'timestamp': stamp}
        if csv_paths:
            checkpoint_info['csv_paths'] = csv_paths
    ...
    def _collect_virtual_model_performance(self, optimizer=None, best_models=None) -> Dict[str, Any]:
        """Collect live virtual-evaluator model metrics for progress rendering."""
        optimizer = optimizer or st.session_state.get('optimizer')
        evaluator = getattr(optimizer, 'evaluator', None) if optimizer is not None else None
        if evaluator is None:
            evaluator = st.session_state.get('evaluator')

        evaluator_type = getattr(optimizer, 'evaluator_type', None) if optimizer is not None else None
        if evaluator_type != EvaluatorType.VIRTUAL or evaluator is None or not getattr(evaluator, 'metrics', {}):
            return {}

        rows = evaluator.get_model_performance_data() if hasattr(evaluator, 'get_model_performance_data') else []
        if not rows:
            return {}

        resolved_best_models: Dict[str, str] = {}
        if isinstance(best_models, dict) and best_models:
            resolved_best_models = {str(k): str(v) for k, v in best_models.items()}
        else:
            grouped_rows: Dict[str, List[Dict[str, Any]]] = {}
            for row in rows:
                grouped_rows.setdefault(str(row.get('objective')), []).append(row)
            for objective, objective_rows in grouped_rows.items():
                best_row = max(
                    objective_rows,
                    key=lambda item: (
                        float(item.get('quality_score', 0.0)),
                        float(item.get('test_r2', float('-inf'))),
                    ),
                )
                resolved_best_models[objective] = str(best_row.get('model'))

        row_lookup = {
            (str(row.get('objective')), str(row.get('model'))): row
            for row in rows
        }
        best_rows: List[Dict[str, Any]] = []
        for objective, model_name in resolved_best_models.items():
            selected = row_lookup.get((objective, model_name))
            if not selected:
                continue
            best_rows.append({
                'objective': objective,
                'model': model_name,
                'test_r2': float(selected.get('test_r2', 0.0)),
                'test_rmse': float(selected.get('test_rmse', 0.0)),
                'quality_score': float(selected.get('quality_score', 0.0)),
                'meets_constraints': bool(selected.get('meets_constraints', False)),
            })

        return {
            'rows': rows,
            'best_models': resolved_best_models,
            'best_rows': best_rows,
            'updated_at': time.time(),
        }

    def _update_virtual_model_performance_state(
        self,
        ...
        """Persist the latest virtual-evaluator metrics in progress state."""
        snapshot = self._collect_virtual_model_performance(optimizer=optimizer, best_models=best_models)
        if not snapshot:
            return

        current_progress['model_performance'] = snapshot
        if not record_history:
            return

        history = current_progress.setdefault('model_performance_history', [])
        candidate_count = int(current_progress.get('candidates_completed', 0))
        batch_no = int(current_progress.get('current_batch', 0))
        for row in snapshot.get('best_rows', []):
            history.append({
                'candidate': candidate_count,
                'batch': batch_no,
                'objective': str(row.get('objective')),
                'model': str(row.get('model')),
                'test_r2': float(row.get('test_r2', 0.0)),
                'test_rmse': float(row.get('test_rmse', 0.0)),
                'quality_score': float(row.get('quality_score', 0.0)),
                'meets_constraints': bool(row.get('meets_constraints', False)),
            })
    ...
    def _update_progress_display(self, progress_container):
        ...
        with progress_container.container():
            ...
            snapshot = progress.get('model_performance', {})
            perf_rows = snapshot.get('rows', []) if isinstance(snapshot, dict) else []
            if perf_rows and optimizer is not None and getattr(optimizer, 'evaluator_type', None) == EvaluatorType.VIRTUAL:
                st.subheader("Virtual Evaluator Performance")

                best_rows = snapshot.get('best_rows', [])
                if best_rows:
                    best_df = pd.DataFrame(best_rows).rename(columns={
                        'objective': 'Objective',
                        'model': 'Best Model',
                        'test_r2': 'Test R2',
                        'test_rmse': 'Test RMSE',
                        'quality_score': 'Quality',
                        'meets_constraints': 'Constraints OK',
                    })
                    st.caption("Current best surrogate per objective")
                    st.dataframe(best_df, use_container_width=True)

                perf_df = pd.DataFrame(perf_rows)
                if not perf_df.empty:
                    display_cols = [
                        'objective', 'model', 'train_r2', 'test_r2',
                        'train_rmse', 'test_rmse', 'quality_score',
                        'cross_val_score', 'meets_constraints',
                    ]
                    display_cols = [c for c in display_cols if c in perf_df.columns]
                    perf_df = perf_df[display_cols].rename(columns={
                        'objective': 'Objective',
                        'model': 'Model',
                        'train_r2': 'Train R2',
                        'test_r2': 'Test R2',
                        ...
                        'test_rmse': 'Test RMSE',
                        'quality_score': 'Quality',
                        'cross_val_score': 'CV R2',
                        'meets_constraints': 'Constraints OK',
                    })
                    st.dataframe(perf_df, use_container_width=True)

                perf_hist = progress.get('model_performance_history', [])
                if perf_hist:
                    perf_hist_df = pd.DataFrame(perf_hist)
                    if not perf_hist_df.empty and perf_hist_df['candidate'].nunique() > 1:
                        fig_r2 = px.line(
                            perf_hist_df,
                            x='candidate',
                            y='test_r2',
