...
using the gcode/relay logic, reads RGB from the Arduino, and returns it
as objective values. GUI is removed; fully headless.
"""
...
home_pos_z = 480.0
safe_height_z = 320
sensor_measure_y = float(os.getenv("SDL_SENSOR_MEASURE_Y", "270"))
...
def unity_send_sensor_rgb(r: float, g: float, b: float, color_name: str = "") -> None:
    _publish_to_unity(
        {
            "type": "sensor_rgb",
...
_LAST_SENSOR_RGB: Optional[Tuple[float, float, float]] = None
_LAST_SENSOR_COLOR: str = ""
_LAST_SENSOR_TS: float = 0.0

# Arduino line format:
# "RGB: R=123 G=88 B=42 | Color: Orange"
# "RGB: 123,88,42 | Color: Orange"
_RGB_RE_LABELED = re.compile(
    r"RGB:\s*R\s*[:=]\s*(\d{1,3})\s*[, ]+\s*G\s*[:=]\s*(\d{1,3})\s*[, ]+\s*B\s*[:=]\s*(\d{1,3})(?:\s*\|\s*Color:\s*(.*))?\s*$",
    re.IGNORECASE,
)
_RGB_RE_COMPACT = re.compile(
    r"RGB:\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})(?:\s*\|\s*Color:\s*(.*))?\s*$",
    re.IGNORECASE,
)
...
def _parse_rgb_line(line: str) -> Optional[Tuple[int, int, int, str]]:
    raw = str(line or "").strip()
    if not raw:
        return None

    match = _RGB_RE_LABELED.search(raw)
    if match is None:
        match = _RGB_RE_COMPACT.search(raw)
    if match is None:
        return None

    r = _clamp_rgb_channel(match.group(1))
    g = _clamp_rgb_channel(match.group(2))
    b = _clamp_rgb_channel(match.group(3))
    color_name = str(match.group(4)).strip() if match.lastindex and match.group(4) else ""
            ...
            with ser:
                while not self._stop.is_set():
                    try:
                        ...
                        parsed = _parse_rgb_line(line)
                        if not parsed:
                            continue

                        red, green, blue, cname = parsed

                        now = time.time()
                        with _SENSOR_LOCK:
                            global _LAST_SENSOR_RGB, _LAST_SENSOR_COLOR, _LAST_SENSOR_TS
                            _LAST_SENSOR_RGB = (float(red), float(green), float(blue))
                            _LAST_SENSOR_COLOR = cname
                            _LAST_SENSOR_TS = now

                        # ✅ broadcast to Unity ONCE PER SECOND
                        if unity_stream_enabled() and (now - last_sent) >= 1.0:
                            unity_send_sensor_rgb(float(red), float(green), float(blue), cname)
                            last_sent = now
...
def read_rgb_once(timeout: float = 1800.0) -> Optional[Tuple[float, float, float]]:
    """
    Read one RGB tuple from Arduino cache.
    """
    deadline = time.time() + timeout
    while time.time() < deadline:
        with _SENSOR_LOCK:
            if _LAST_SENSOR_RGB is not None and (time.time() - _LAST_SENSOR_TS) < 3.0:
                r, g, b = _LAST_SENSOR_RGB
                return float(r), float(g), float(b)
        time.sleep(0.05)
    return None


def _read_cached_rgb(max_age_sec: float = 3.0) -> Optional[Tuple[float, float, float, str, float]]:
    max_age = max(0.05, float(max_age_sec))
    now = time.time()
    with _SENSOR_LOCK:
        if _LAST_SENSOR_RGB is None:
            return None
        if (now - _LAST_SENSOR_TS) > max_age:
            return None
        red, green, blue = _LAST_SENSOR_RGB
        return (
            float(red),
            float(green),
            float(blue),
            str(_LAST_SENSOR_COLOR),
            float(_LAST_SENSOR_TS),
        )
...
def read_rgb_stats(
    ...
    while len(samples) < requested and time.time() < deadline:
        cached = _read_cached_rgb(max_age_sec=max_age)
        if cached is None:
            time.sleep(0.02)
            continue

        red, green, blue, color_name, rgb_ts = cached
        if require_fresh and last_ts is not None and rgb_ts <= (last_ts + 1e-9):
            time.sleep(0.01)
            continue

        samples.append((float(red), float(green), float(blue)))
        sample_timestamps.append(float(rgb_ts))
        latest_color = str(color_name)
        last_ts = float(rgb_ts)
    ...
    channels: Dict[str, List[float]] = {
        "R": [sample[0] for sample in samples],
        "G": [sample[1] for sample in samples],
        "B": [sample[2] for sample in samples],
    }
    mean_map: Dict[str, float] = {}
    std_map: Dict[str, float] = {}
    sem_map: Dict[str, float] = {}
    for channel_name, values in channels.items():
        mean_value = float(sum(values) / float(n))
        if n >= 2:
            variance = sum((x - mean_value) ** 2 for x in values) / float(n - 1)
            std_value = float(math.sqrt(max(variance, 0.0)))
            sem_value = float(std_value / math.sqrt(float(n)))
        else:
            std_value = 0.0
            sem_value = 0.0
        mean_map[channel_name] = mean_value
        std_map[channel_name] = std_value
        sem_map[channel_name] = sem_value
...
def read_frequency_once(timeout: float = 1800.0) -> Optional[float]:
    """
    Compatibility wrapper: returns average RGB intensity from one cached sample.
    """
    rgb = read_rgb_once(timeout=timeout)
    if rgb is None:
        return None
    red, green, blue = rgb
...
def read_frequency_stats(
    ...
    """
    Compatibility wrapper: converts RGB replicate stats into average intensity stats.
    """
    rgb_stats = read_rgb_stats(
        sample_count=sample_count,
        timeout=timeout,
        sample_interval=sample_interval,
        require_fresh_samples=require_fresh_samples,
        max_age_sec=max_age_sec,
    )
    if rgb_stats is None:
        return None

    samples_rgb = rgb_stats.get("samples", [])
    intensity_samples: List[float] = []
    for sample in samples_rgb:
        if not isinstance(sample, dict):
            continue
        red = _coerce_nonnegative_float(sample.get("R"), 0.0)
        green = _coerce_nonnegative_float(sample.get("G"), 0.0)
        blue = _coerce_nonnegative_float(sample.get("B"), 0.0)
        intensity_samples.append(float((red + green + blue) / 3.0))

    if not intensity_samples:
        return None

    n = len(intensity_samples)
    mean_value = float(sum(intensity_samples) / float(n))
    if n >= 2:
        variance = sum((x - mean_value) ** 2 for x in intensity_samples) / float(n - 1)
        std_value = float(math.sqrt(max(variance, 0.0)))
        sem_value = float(std_value / math.sqrt(float(n)))
    else:
        std_value = 0.0
    ...
    return {
        ...
        "std": std_value,
        "sem": sem_value,
        "samples": intensity_samples,
        "timestamps": [float(v) for v in rgb_stats.get("timestamps", [])],
        "n_samples": int(rgb_stats.get("n_samples", n)),
        "n_requested": int(rgb_stats.get("n_requested", sample_count)),
        "latest_color": str(rgb_stats.get("latest_color", "")),
        "incomplete": bool(rgb_stats.get("incomplete", False)),
    }
...
def run_mixing_process(
    ...
    if STOP_REQUESTED:
        ...
        move_to(z=home_pos_z, unity_evts=["MOVE"])
        move_to(x=0.0, y=0.0, unity_evts=["COMPLETE"])
...
class SDLHardwareAgent:
    ...
    def _run_command(self, cmd: str, args: Dict[str, Any]) -> None:
        ...
        if cmd_norm == "home":
            move_to(z=home_pos_z, feedrate=2000, unity_evts=["MOVE"])
    ...
    def _measure_rgb_payload(self, trial_index: Optional[Any]) -> Dict[str, Any]:
        logger.info(
            "[SDL] moving to measurement Y position y=%.2f before RGB sampling (trial=%s)",
            sensor_measure_y,
            trial_index,
        )
        move_to(y=sensor_measure_y, unity_evts=["MEASURE_POS"])

        try:
            post_wait_sec = max(0.0, float(getattr(self.args, "sensor_post_wait_sec", 0.0)))
            if post_wait_sec > 0.0:
                logger.info(
                    "[SDL] post-trial wait %.2fs before RGB sampling (trial=%s)",
                    post_wait_sec,
                    trial_index,
                )
                wait_deadline = time.time() + post_wait_sec
            ...
            if stats is None:
                raise RuntimeError("No RGB reading from sensor")
            ...
            if bool(stats.get("incomplete", False)) and not allow_partial:
                raise RuntimeError(
                    f"Incomplete RGB replicate set ({n_samples}/{requested}) and partial samples are disabled"
                )

            mean_raw = stats.get("mean", {})
            sem_raw = stats.get("sem", {})
            ...
            for channel_name in channel_names:
                objectives[channel_name] = float(_coerce_nonnegative_float((mean_raw or {}).get(channel_name), 0.0))
                sem_map[channel_name] = abs(float(_coerce_nonnegative_float((sem_raw or {}).get(channel_name), 0.0)))
                std_map[channel_name] = abs(float(_coerce_nonnegative_float((std_raw or {}).get(channel_name), 0.0)))

            if n_samples < 2:
                for channel_name in channel_names:
                    sem_map[channel_name] = max(float(sem_map[channel_name]), fallback_sem)
                    std_map[channel_name] = max(float(std_map[channel_name]), fallback_sem)

            spread_mode = str(getattr(self.args, "sensor_uncertainty_type", "sem")).strip().lower()
            if spread_mode in {"std", "stdev", "sigma"}:
                uncertainty_type = "std"
                uncertainty_map = std_map
            else:
                uncertainty_type = "sem"
                uncertainty_map = sem_map

            if bool(stats.get("incomplete", False)):
                logger.warning(
                    "[SDL] RGB replicates incomplete for trial %s: collected=%s requested=%s",
                    trial_index,
                    n_samples,
                    requested,
                )
            ...
            return {
                "objectives": {
                    "R": float(objectives["R"]),
                    "G": float(objectives["G"]),
                    "B": float(objectives["B"]),
                },
                "objective_uncertainties": {
                    "R": float(uncertainty_map["R"]),
                    "G": float(uncertainty_map["G"]),
                    "B": float(uncertainty_map["B"]),
                },
                "uncertainty_type": uncertainty_type,
                "measurement_metadata": {
                    "rgb_samples_collected": n_samples,
                    "rgb_samples_requested": requested,
                    "rgb_sem": {
                        "R": float(sem_map["R"]),
                        "G": float(sem_map["G"]),
                        "B": float(sem_map["B"]),
                    ...
                    "rgb_latest_color": str(stats.get("latest_color", "")),
                },
            }
        finally:
            logger.info(
                "[SDL] returning to home position x=0 y=0 z=%.2f after RGB sampling (trial=%s)",
                home_pos_z,
                trial_index,
            )
            move_to(x=0.0, y=0.0, z=home_pos_z, unity_evts=["MEASURE_DONE"])
    ...
    def _run_automatic_candidate(self, trial_index: Optional[Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        measurement_payload = self._measure_rgb_payload(trial_index)
    ...
    def _run_manual_candidate(self, trial_index: Optional[Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            measurement_payload = self._measure_rgb_payload(manual_trial_index)
...
def parse_args():
    ...
    parser.add_argument("--sensor-timeout", type=float, default=30.0, help="Seconds to wait for sensor RGB reading")
    parser.add_argument(
        "--sensor-replicates",
        type=int,
        default=int(os.getenv("SDL_SENSOR_REPLICATES", "3")),
        help="Number of repeated RGB samples per trial for uncertainty estimation.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sensor-post-wait-sec",
        type=float,
        default=float(os.getenv("SDL_SENSOR_POST_WAIT_SEC", "0.0")),
        help="Optional delay (seconds) after trial completion before collecting RGB replicates.",
    )
    parser.add_argument(
        "--sensor-max-age-sec",
        type=float,
...
if __name__ == "__main__":
    ...
    logger.info(
        "[SENSOR] uncertainty config replicates=%s interval=%ss post_wait=%ss max_age=%ss require_fresh=%s allow_partial=%s type=%s fallback_sem=%s",
        max(1, int(args.sensor_replicates)),
        max(0.0, float(args.sensor_sample_interval)),
        max(0.0, float(getattr(args, "sensor_post_wait_sec", 0.0))),
    ...
    # Start sensor streaming always so Unity receives periodic RGB updates.
